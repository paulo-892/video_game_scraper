...
def lambda_handler(event, context):
    ...
    get_secret_value_response = client.get_secret_value(SecretId='google-api-secrets')
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Successfully retrieved secret')
    }
...
def authorize_google_api(dict):
    scope = [
        'https://www.googleapis.com/auth/drive',
        'https://www.googleapis.com/auth/drive.file'
    ]
    file_name = 'client_key.json'
    creds = ServiceAccountCredentials.from_json_keyfile_dict(dict)
    return gspread.authorize(creds)


def test(secret_json):
    # authorizes Google Drive API
    client = authorize_google_api(secret_json)
